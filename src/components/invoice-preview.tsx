...
export function InvoicePreview({ invoiceData, subtotal }: InvoicePreviewProps) {
  const {
    companyDetails,
    clientDetails,
    invoiceItems,
      ...
      <div className="flex justify-between mb-8">
        <div>
          ...
          <div className="font-medium">
            {invoiceMetadata.invoiceNumber || "000001"}
          </div>
        </div>
        <div className="flex gap-8">
          <div>
            ...
            <div className="font-medium">
              {invoiceMetadata.issueDate || "3/24/25"}
            </div>
          </div>
          <div>
            <div className="text-xs text-gray-500">DUE DATE</div>
            <div className="font-medium">
              {invoiceMetadata.dueDate || "4/07/25"}
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="border border-gray-200 rounded-md p-4">
              ...
              <div className="text-sm text-gray-600 mt-1">
                ...
                {companyDetails.email && (
                  <div className="mt-2">{companyDetails.email}</div>
                )}
            ...
            <div className="border border-gray-200 rounded-md p-4">
              ...
              <div className="text-sm text-gray-600 mt-1">
                {clientDetails.clientAddress && (
                  <div>{clientDetails.clientAddress}</div>
                )}
                ...
                {clientDetails.clientEmail && (
                  <div className="mt-2">{clientDetails.clientEmail}</div>
                )}
              </div>
            </div>
          ) : (
        ...
        <div className="w-[45%]">
          ...
          <div className="text-sm text-gray-600">
            ...
            <div className="font-medium break-all">
              {paymentDetails.walletAddress || "Enter wallet address"}
            </div>
          </div>
        </div>
      </div>
